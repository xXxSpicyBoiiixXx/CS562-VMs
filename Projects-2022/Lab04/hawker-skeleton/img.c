...
hkr_clear_img_cache (void)
{
    ...
    dp = opendir(hkr_get_img_path());

    if (!dp) {
        ERRSTR("Couldn't open images dir");
        return;
    }
    ...
    while ((e = readdir(dp))) {
        if (e->d_type == DT_DIR && e->d_name[0] != '.') {
            ...
            char ptspath[PATH_MAX];
            char cmd[PATH_MAX];
            memset(cmd, 0, PATH_MAX);
            memset(devpath, 0, PATH_MAX);
            memset(ptspath, 0, PATH_MAX);
            snprintf(cmd, PATH_MAX, "rm -rf %s/%s", hkr_get_img_path(), e->d_name);
            snprintf(devpath, PATH_MAX, "%s/%s/dev", hkr_get_img_path(), e->d_name);
            snprintf(ptspath, PATH_MAX, "%s/%s/dev/pts", hkr_get_img_path(), e->d_name);
            
            printf("%s\n", e->d_name);

            //printf("unmounting %s\n", ptspath);
            if (umount(ptspath) != 0) {
                ERRSTR("could not unmount /dev/pts for container");
            }

            //printf("unmounting %s\n", devpath);
            if (umount(devpath) != 0) {
                ERRSTR("could not unmount /dev for container");
            }

            //printf("running %s\n", cmd);
            if (system(cmd) != 0) {
                ERRSTR("could not remove container directory");
            }
        }
    }
